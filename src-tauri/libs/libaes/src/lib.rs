// Copyright 2020 Han Xu
//
// This Rust implementation is ported from the C version in OpenSSL 1.1.1 stable AES core.
// Please see README.md for license info.

//! A safe and small Rust implementation of AES cipher.
//!
//! This Rust implementation is ported from the C implementation in [OpenSSL 1.1.1 stable](https://github.com/openssl/openssl/blob/OpenSSL_1_1_1-stable/crypto/aes/aes_core.c)
...
const RD_KEY_MAX_SIZE: usize = 4 * (AES_MAX_ROUNDS + 1);
...
impl Cipher {
    ...
    /// let cipher = Cipher::new_128(my_key);
    /// ```
    pub fn new_128(user_key: &[u8; AES_128_KEY_LEN]) -> Cipher {
    ...
    pub fn cbc_decrypt(&self, iv: &[u8], data: &[u8]) -> Vec<u8> {
        ...
        for i in (0..length).step_by(AES_BLOCK_SIZE) {
            ...
            aes_decrypt(block, &self.decrypt_key);
            xor_with_iv(block, my_iv);
    ...
    pub fn ebc_decrypt(&self, data: &[u8]) -> Vec<u8> {
        let length = data.len();
        if (length % AES_BLOCK_SIZE) != 0 {
            return Vec::new(); // data is invalid.
        }
        ...
        for block in new.chunks_exact_mut(AES_BLOCK_SIZE) {
            aes_decrypt(block, &self.decrypt_key);
        }
...
// `bytes` must have at least 4 bytes.
fn get_u32(bytes: &[u8]) -> u32 {
    let u8_array: [u8; 4] = [bytes[0], bytes[1], bytes[2], bytes[3]];
    u32::from_be_bytes(u8_array)
}

fn put_u32(bytes: &mut [u8], val: u32) {
    let u8_array: [u8; 4] = u32::to_be_bytes(val);
    bytes.copy_from_slice(&u8_array);
}

struct AesKey {
    pub rd_key: [u32; RD_KEY_MAX_SIZE], // big enough for any key length
    pub rounds: u8,
}
...
fn aes_set_encrypt_key(user_key: &[u8], key_bits: u16, key: &mut AesKey) {
    key.rounds = match key_bits {
        ...
        192 => 12,
        256 => 14,
        _ => panic!("wrong value for bits {}", key_bits),
    };

    let mut rk = &mut key.rd_key[..];
    rk[0] = get_u32(&user_key[0..4]);
    rk[1] = get_u32(&user_key[4..8]);
    rk[2] = get_u32(&user_key[8..12]);
    rk[3] = get_u32(&user_key[12..16]);

    if key_bits == 128 {
        let mut i = 0;
        loop {
            let temp: usize = rk[3] as usize;
            rk[4] = rk[0]
    ...
    rk[4] = get_u32(&user_key[16..20]);
    rk[5] = get_u32(&user_key[20..24]);

    if key_bits == 192 {
        let mut i = 0;
        loop {
            let temp: usize = rk[5] as usize;
            rk[6] = rk[0]
                ^ (TE2[(temp >> 16) & 0xff] & 0xff000000)
                ^ (TE3[(temp >> 8) & 0xff] & 0x00ff0000)
                ^ (TE0[(temp) & 0xff] & 0x0000ff00)
    ...
    rk[6] = get_u32(&user_key[24..28]);
    rk[7] = get_u32(&user_key[28..32]);

    if key_bits == 256 {
        let mut i = 0;
        loop {
            let temp: usize = rk[7] as usize;
            rk[8] = rk[0]
                ^ (TE2[(temp >> 16) & 0xff] & 0xff000000)
                ^ (TE3[(temp >> 8) & 0xff] & 0x00ff0000)
                ^ (TE0[(temp) & 0xff] & 0x0000ff00)
...
fn aes_set_decrypt_key(user_key: &[u8], key_bits: u16, key: &mut AesKey) {
    ...
    let mut i: usize = 0;
    let mut j: usize = 4 * key.rounds as usize;
    while i < j {
        temp = rk[i];
        rk[i] = rk[j];
        rk[j] = temp;
    ...
    for _ in 1..key.rounds {
        rk = &mut rk[4..];
        rk[0] = TD0[TE1[(rk[0] >> 24) as usize] as usize & 0xff]
            ^ TD1[TE1[(rk[0] >> 16) as usize & 0xff] as usize & 0xff]
            ^ TD2[TE1[(rk[0] >> 8) as usize & 0xff] as usize & 0xff]
            ^ TD3[TE1[(rk[0]) as usize & 0xff] as usize & 0xff];
        rk[1] = TD0[TE1[(rk[1] >> 24) as usize] as usize & 0xff]
            ^ TD1[TE1[(rk[1] >> 16) as usize & 0xff] as usize & 0xff]
            ^ TD2[TE1[(rk[1] >> 8) as usize & 0xff] as usize & 0xff]
            ^ TD3[TE1[(rk[1]) as usize & 0xff] as usize & 0xff];
        rk[2] = TD0[TE1[(rk[2] >> 24) as usize] as usize & 0xff]
            ^ TD1[TE1[(rk[2] >> 16) as usize & 0xff] as usize & 0xff]
            ^ TD2[TE1[(rk[2] >> 8) as usize & 0xff] as usize & 0xff]
            ^ TD3[TE1[(rk[2]) as usize & 0xff] as usize & 0xff];
        rk[3] = TD0[TE1[(rk[3] >> 24) as usize] as usize & 0xff]
            ^ TD1[TE1[(rk[3] >> 16) as usize & 0xff] as usize & 0xff]
            ^ TD2[TE1[(rk[3] >> 8) as usize & 0xff] as usize & 0xff]
            ^ TD3[TE1[(rk[3]) as usize & 0xff] as usize & 0xff];
    }
}
...
fn aes_encrypt(block_buf: BlockBuf, key: &AesKey) {
    ...
    let (mut s0, mut s1, mut s2, mut s3, output) = match block_buf {
        BlockBuf::InPlace(buf) => (
            get_u32(&buf[0..4]) ^ rk[0],
            get_u32(&buf[4..8]) ^ rk[1],
            get_u32(&buf[8..12]) ^ rk[2],
            get_u32(&buf[12..16]) ^ rk[3],
            buf,
        ),
        BlockBuf::FromTo((in_buf, out_buf)) => (
            get_u32(&in_buf[0..4]) ^ rk[0],
    ...
    loop {
        ...
        s0 = TE0[(t0 >> 24) as usize]
            ^ TE1[(t1 >> 16) as usize & 0xff]
            ^ TE2[(t2 >> 8) as usize & 0xff]
            ^ TE3[(t3) as usize & 0xff]
            ^ rk[0];
        s1 = TE0[(t1 >> 24) as usize]
            ^ TE1[(t2 >> 16) as usize & 0xff]
            ^ TE2[(t3 >> 8) as usize & 0xff]
            ^ TE3[(t0) as usize & 0xff]
            ^ rk[1];
        s2 = TE0[(t2 >> 24) as usize]
            ^ TE1[(t3 >> 16) as usize & 0xff]
            ^ TE2[(t0 >> 8) as usize & 0xff]
            ^ TE3[(t1) as usize & 0xff]
            ^ rk[2];
        s3 = TE0[(t3 >> 24) as usize]
            ^ TE1[(t0 >> 16) as usize & 0xff]
            ^ TE2[(t1 >> 8) as usize & 0xff]
            ^ TE3[(t2) as usize & 0xff]
            ^ rk[3];
    }
    ...
    let s2 = (TE2[(t2 >> 24) as usize] & 0xff000000)
    ...
    let s3 = (TE2[(t3 >> 24) as usize] & 0xff000000)
...
fn aes_decrypt(buf: &mut [u8], key: &AesKey) {
    let mut rk: &[u32] = &key.rd_key[..];
    ...
    let u8_4: [u8; 4] = [
        TD4[(t0 >> 24) as usize],
        TD4[(t3 >> 16) as usize & 0xff],
        TD4[(t2 >> 8) as usize & 0xff],
        TD4[(t1) as usize & 0xff],
    ];
    let s0 = u32::from_be_bytes(u8_4) ^ rk[0];
    put_u32(&mut buf[0..4], s0);

    let u8_4: [u8; 4] = [
        TD4[(t1 >> 24) as usize],
        TD4[(t0 >> 16) as usize & 0xff],
        TD4[(t3 >> 8) as usize & 0xff],
        TD4[(t2) as usize & 0xff],
    ];
    let s1 = u32::from_be_bytes(u8_4) ^ rk[1];
    put_u32(&mut buf[4..8], s1);

    let u8_4: [u8; 4] = [
        TD4[(t2 >> 24) as usize],
        TD4[(t1 >> 16) as usize & 0xff],
        TD4[(t0 >> 8) as usize & 0xff],
        TD4[(t3) as usize & 0xff],
    ];
    let s2 = u32::from_be_bytes(u8_4) ^ rk[2];
    put_u32(&mut buf[8..12], s2);

    let u8_4: [u8; 4] = [
        TD4[(t3 >> 24) as usize],
        TD4[(t2 >> 16) as usize & 0xff],
        TD4[(t1 >> 8) as usize & 0xff],
        TD4[(t0) as usize & 0xff],
    ];
    let s3 = u32::from_be_bytes(u8_4) ^ rk[3];
